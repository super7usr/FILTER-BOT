...
async def get_all_search_results(query, file_type=None, offset=0, filter=False):
    """For given query return (results, next_offset)"""
    ...
    files = []
    if MULTIPLE_DATABASE:
        cursor1 = col.find(filter).sort('$natural', -1).skip(offset)
        cursor2 = sec_col.find(filter).sort('$natural', -1).skip(offset)
    ...
    total_results = col.count_documents(filter) if not MULTIPLE_DATABASE else (col.count_documents(filter) + sec_col.count_documents(filter))
    next_offset = "" if offset >= total_results else offset
